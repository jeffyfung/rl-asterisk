...
class SampleBuffer():
    ...
    # compute and store the GAE lambda advantage and the return; called at the end of an episode
    def compute_adv_and_rtn(self, gamma, lam, final_val, final_term):
        ...
        for s in reversed(range(self.buffer_width)):
            ...
            delta = self.rew_buf[s] + next_step_non_term_mask * \
                gamma * next_step_val - self.val_buf[s]
            prev_adv = 0 if s == self.buffer_width - 1 else self.adv_buf[s + 1]
            self.adv_buf[s] = delta + \
                next_step_non_term_mask * gamma * lam * prev_adv
